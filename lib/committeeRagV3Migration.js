export async function ensureCommitteeRagV3Tables(prisma) {
  try {
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS "committee_documents" (
        "id" UUID PRIMARY KEY,
        "meetingId" INTEGER NOT NULL REFERENCES "committee_meetings"("id") ON DELETE CASCADE,
    ...
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS "committee_document_pages" (
        ...
        "documentId" UUID NOT NULL REFERENCES "committee_documents"("id") ON DELETE CASCADE,
    ...
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS "committee_ingestion_jobs" (
        ...
        "nextRetryAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
        "error" TEXT,
        "payload" JSONB,
        "createdAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
        "updatedAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP
    ...
    console.error('ensureCommitteeRagV3Tables failed:', err.message);
  }
}
