...
export function splitIntoSemanticChunks(text, {
  ...
  if (!cleanText) return [];
  if (!Number.isInteger(maxSize) || maxSize < 2 || overlap < 0 || overlap >= maxSize) {
    throw new Error('청크 크기는 2 이상이며 중첩보다 커야 합니다.');
  }
  if (cleanText.length <= maxSize) return [cleanText];

  // Reserve the overlap budget before splitting, never truncate source text afterwards.
  const contentBudget = maxSize - overlap - 1;
  const units = semanticUnits(cleanText, Math.max(1, contentBudget));
  const baseChunks = [];
  let current = '';
  ...
  for (const unit of units) {
    ...
    current = candidate;
    if (current.length >= Math.min(targetSize, contentBudget)) {
      baseChunks.push(current.trim());
      current = '';
    }
  }
  ...
  if (current) {
    if (baseChunks.length > 0 && current.length < minSize && baseChunks.at(-1).length + current.length + 1 <= contentBudget) {
