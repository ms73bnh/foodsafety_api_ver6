...
// 마이그레이션/대량 작업 시에는 직결 URL(UNPOOLED)을 사용하여 풀러 타임아웃 방지
...
async function fetchArticlesFromPage(pageNum, headers) {
  const url = `${BASE_URL_SYNC}/brd/m_532/list.do?page=${pageNum}`;
...
async function syncAllCommittee() {
  console.log('=== 1. Collecting all articles from MFDS ===');
  const headers = {
  ...
  for (let page = 1; page <= 40; page++) {
    ...
    const articles = await fetchArticlesFromPage(page, headers);
    if (articles.length === 0) {
      console.log('No more articles.');
      break;
    }
    console.log(`found ${articles.length} meetings.`);
    allArticles.push(...articles);
  }

  console.log(`\n=== 2. Total articles collected: ${allArticles.length} ===`);
  console.log('=== 3. Saving meetings & agendas to DB ===');
  ...
  for (let i = 0; i < allArticles.length; i++) {
    ...
    try {
      ...
      if (!existing) {
        ...
        savedCount++;
        console.log(`[${i + 1}/${allArticles.length}] Created: ${art.title} (Agendas: ${agendas.length})`);
      } else {
        console.log(`[${i + 1}/${allArticles.length}] Already exists: ${art.title}`);
      }
    } catch (e) {
      console.error(`Error saving ${art.seq}:`, e.message);
    }
  }

  const meetingCount = await prisma.committee_meetings.count();
  const agendaCount = await prisma.committee_agendas.count();
  console.log(`\n=== Done! Total Meetings in DB: ${meetingCount}, Total Agendas in DB: ${agendaCount} ===`);
}
